...
```bash
curl -fsSLO https://github.com/zcimon57-svj/claude-code-offline/releases/download/v2.1.119/claude-code-offline-2.1.119-linux-x64.tar.gz
```
...
```bash
ls -lh claude-code-offline-2.1.119-linux-x64.tar.gz   # 大小约 74M
tar -tzf claude-code-offline-2.1.119-linux-x64.tar.gz | head
# 期望看到 claude-code-offline-2.1.119-linux-x64/{claude,install.sh,LICENSE.md,README.txt}
```
...
把 `claude-code-offline-2.1.119-linux-x64.tar.gz` 这**一个文件**拷到内网机即可（scp / U盘 / 内部网盘 / 跳板机）。
...
```bash
tar -xzf claude-code-offline-2.1.119-linux-x64.tar.gz
cd claude-code-offline-2.1.119-linux-x64
./install.sh
```
...
claude --version          # 期望: 2.1.119 (Claude Code)
